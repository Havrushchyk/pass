...
# Keras 
for loop1 in range(98,99): # 98 128
  for loop2 in range(1,2): # Best
    ...
    def create_baseline():
        # create model
        model = Sequential()
        model.add(Dense(loop1,bias_initializer='random_uniform', input_dim=X.shape[1],activation='relu')) #  kernel_initializer='random_uniform',bias_initializer='random_uniform', 
        model.add(Dense(loop1, activation='relu'))
        model.add(Dense(loop1, activation='relu'))
    ...
    # Fit the model
    early_stopping=EarlyStopping(monitor='loss',min_delta=0.0001, patience=500, verbose=0 ,mode='auto') # min_delta=0.001,
    ...
    """
    best_acc = h5py.File(filepath, 'r')
    
    # List all groups
    print("Keys: %s" % best_acc.keys())
    a_group_key = list(best_acc.keys())[0]

    # Get the data
    data = list(best_acc[a_group_key])
    ...
    model.fit(X, y.ravel(), epochs=10000, batch_size=batch_size, verbose=1, callbacks=callbacks_list, shuffle=False) # validation_split=0.2,   callbacks=[early_stopping],
    ...
    # Load best weigth
    model2 = Sequential()
    model2.add(Dense(loop1,bias_initializer='random_uniform', input_dim=X.shape[1],activation='relu')) # kernel_initializer='random_uniform', bias_initializer='random_uniform',
    model2.add(Dense(loop1, activation='relu'))
    model2.add(Dense(loop1, activation='relu'))
